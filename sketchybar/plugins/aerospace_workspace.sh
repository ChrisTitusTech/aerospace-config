...
for i in {1..8}; do
  HAS_WINDOWS=$(aerospace list-windows --workspace "$i" 2>/dev/null | wc -l | tr -d ' ')
  if [ "$i" = "$FOCUSED" ]; then
    ARGS+=(--set "space.$i" "label=${i}${U}" background.color=0xff81a1c1 label.color=0xff2e3440 drawing=on)
  elif [ "$HAS_WINDOWS" -gt 0 ]; then
    ARGS+=(--set "space.$i" "label=$i" background.color=0xff3b4252 label.color=0xffd8dee9 drawing=on)
  else
